...
> Push a `v*` tag → auto-publish to 5 major skill registries.
...
| **SkillHub** (腾讯) | 国内加速镜像 | `clawhub publish` CLI (自定义 registry) | `SKILLHUB_TOKEN` + `SKILLHUB_REGISTRY` |
...
2. **并行发布** → ClawHub / SkillHub / agentskill.sh（各自 continue-on-error，互不影响）
...
| `SKILLHUB_TOKEN` | SkillHub 个人控制台 → 创建 API Token | SkillHub 发布 |
| `SKILLHUB_REGISTRY` | 通常为 `https://skillhub.cloud.tencent.com`，企业自建可自定义 | SkillHub 发布 |
...
| SkillHub | `https://skillhub.cloud.tencent.com/skills/tianji` |
...
**Q: 只想发布到国内平台（SkillHub）怎么办？**

A: 使用手动触发：Actions → Publish to Registries → Run workflow，`targets` 填 `skillhub`。
