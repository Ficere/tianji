---
name: tianji
description: "天机 —— 综合命理测算技能（v7.1）。一站式完成八字五行、称骨、紫微斗数（14主星+四化+大限）、西洋星座（太阳/月亮/上升）、三才五格姓名测算及多场景合盘。v7.1修正三层归类：太阳星座归内核层（核心意志），上升为外在界面，年月柱归命运方向，MBTI功能栈表述去绝对化；新增评分口径统一、MBTI置信度与管理者伦理边界。当用户提到命理测算、算命、八字、五行、称骨、紫微、星座、合盘、三才五格、姓名测算、姓名打分、名字吉凶、综合测算、一站式测算、天机、怎么和他沟通、如何安排工作、亲子养育建议时使用此技能。" 
metadata:
  author: computer
  version: '7.1.0'
  language: zh-CN
...
- 人生命运主要取决于个人努力和选择
- **[v7.1 勘误记录]** 原v7.0将太阳星座归入「外在表现」层（措辞：「较稳定但受环境影响」）已于v7.1更正：太阳归内核层（核心意志/自我认同）；上升保留外在表现；年柱月柱从外在表现移至命运方向层。评分口径统一：五行平衡20+五行俱全5+生肖20+星座15+日主20+称骨15+姓名5=100，消除旧版「五行互补25+五行俱全+5」计分超出100的bug。
