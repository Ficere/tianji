...
| 五行俱全 | 5 | 是否五行齐全 |
| 生肖关系 | 20 | 六合 / 三合 / 六冲 / 相害（**人对均值**） |
| 星座合盘 | 15 | 相位角度匹配度（**人对均值**） |
| 日主生克 | 20 | 天干合 / 相生 / 相克（**人对均值**） |
| 称骨对比 | 15 | 平均骨重 |
| 姓名合盘 | 5 | 人格五行互补 + 三才配置（仅全员提供姓名时） |
| **总分** | **100** | ★ ~ ★★★★★ |

**两个关键约定（v8.1）**

1. **人对均值，不累加。** 生肖 / 星座 / 日主三项取两两人对得分的均值，
   使 2 人组与 5 人组处于同一量纲、可直接比较。
   （v8.1 之前为累加式，5 人组最高可得 **105** 分、2 人组最高仅 **77** 分，且姓名项从不参与计分。）
2. **未提供全员姓名时，姓名项计 0 分且满分降为 95**（见输出的 `max_possible`），
   评级基于 `total / max_possible` 百分比，避免无姓名组合被系统性压低。

> 本表、`SKILL.md` §9.1、`schemas/reading_v8.schema.json` 与 `analyze_synastry` 的**真实计算输出**
> 由 CI 四方回归校验（`tests/test_suite.py` 对 2/3/5 人随机组合抓挡位与总分），不仅校验示例文件。
...
**输入**：陈明远，男，1992-03-15 14:30，出生城市武汉。

**输出**：八字排盘 + 称骨算命 + 紫微斗数（命宫、主星、四化飞星、大限）+ 星座分析（太阳、月亮、上升三星）+ 姓名五格（人格、地格、总格、三才、综合评级）。

### 案例二：夫妻合盘分析

**输入**：王志远（男，1988-11-23 09:15，成都）+ 李雨薇（女，1990-06-08 19:40，杭州）。

**输出**：两人分别排盘后，计算五行平衡（15/20）+ 五行俱全（5/5）、生肖关系（12/20）、星座相位（5/15）、日主生克（16/20）、称骨对比（12/15）、姓名合盘（3.7/5），总分 68.7/100 → “中等组合”，并给出具体改善建议。
...
**输出**：五格笔画数理、吉凶、五行、三才配置（土金土 → 大吉）、综合评分 82 分 → “吉·上等”，给出性格与发展建议。
